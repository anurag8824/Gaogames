...
// ✅ Setup Redis adapter (important for PM2 cluster mode)
...
Promise.all([pubClient.connect(), subClient.connect()])
  .then(() => {
    ...
    io.use((socket, next) => {
        ...
        // JWT logic can be re-enabled here
        console.log(`Socket authenticated (${socket.id})`);
        next();
    });
    ...
    server.listen(port, () => {
        console.log(`🚀 Server running on http://localhost:${port}`);
        console.log("JWT Secret Loaded:", YOUR_JWT_SECRET ? "Yes" : "NO - Using default!");
    });
  })
  .catch(err => {
